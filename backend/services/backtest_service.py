    ...
    def _simulate_trades(
        ...
        for i, (idx, z) in enumerate(zscore.items()):
            ...
            if signal in ("LONG", "SHORT") and position == "NONE":
                _std_val = spread.rolling(Config.LOOKBACK_WINDOW).std().get(idx)
                spread_std = float(_std_val) if _std_val is not None and not pd.isna(_std_val) else 0.001
                spread_std = max(spread_std, 0.0001)
                # Fixed fractional: risk 2% of capital, max position = capital/price_y * 0.1
                risk_amount = capital * Config.RISK_PER_TRADE
                max_position = (capital / max(price_y, 0.0001)) * 0.1
                quantity = min(risk_amount / spread_std, max_position)
